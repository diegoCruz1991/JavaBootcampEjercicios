public class Strings {
    ...
    public static void main(String... args) {
        ...
        Empleado empleado1 = obtenerEmpleado("  Juan | | Cabrera | Avila ");

        empleado1.segundoApellido = "Avila";

        Empleado empleado2 = obtenerEmpleado("  Juan | Bernardo |  Cabrera | Avila ");

        empleado2.segundoApellido = "Avila";
    ...
    public static Empleado obtenerEmpleado(String registro) { //Recibe un String que es el registro a procesar
        ...
        boolean flagsegundonombre = false; //Falso o verdadero de que no hay segundo nombre
        ...
        for (int i = 0; i < cadenaLimpia.length(); i++) { //Se define un for para iterar sobre la cadena con un indice en una variable llamada i
            if (cadenaLimpia.charAt(i) == '|') {  // Verifica si el caracter leido es un pipe
                ...
                encontrePipe = true; // y Pone en true la bandera de encontrePipe
                if (cadenaLimpia.charAt(i+1) == '|') {
                    pipesContador = pipesContador-1; // Se decrementa el contador y se asigna un caracter vacío en segundo nombre
                    empleado.segundoNombre = "";  //
                    flagsegundonombre = true;  // se enciende la bandera
                }
                ...
                if (encontrePipe) { // Verifica si se encontro un pipe SOLO en la iteracion anterior
                    ...
                    if (flagsegundonombre) {
                        pipesContador = pipesContador+1;
                        flagsegundonombre = false;  //se apaga la bandera y se decrementa el contador
                    }
